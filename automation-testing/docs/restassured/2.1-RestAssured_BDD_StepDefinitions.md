# BDD Mapping — Step Definitions (JUnit runner) & Framework Integration

**Note**: This document describes a BDD/Cucumber approach. For the primary framework approach (pure TestNG-based), refer to **1.1-RestAssured_Framework_Guide.md** and **1.2-RestAssured_Framework_Theory.md**. This BDD pattern is **optional** and complementary to the core TestNG framework.
...
- **Configuration Manager** — Reads environment config
- **RestClient** — HTTP abstraction layer
- **Authentication & Prerequisite Layer** — Token and test data management
- **Utilities, Constants & Enums** — Shared helpers

The primary difference is the **test execution style**: BDD uses human-readable Gherkin scenarios and step definitions instead of TestNG `@Test` methods.
